...
class KzGlobalStats:
    ...
    def embed_stats(self) -> Embed:
        ...
        for i in range(1, 8):
            tp_content += f"{percentage_bar(self.tp_tier_maps[i] / self.maps.tier[i], bar_length=15, fill_char=emojis[i], empty_char='⬛', show_percentage=False, show_brackets=False)}"
        ...
        for i in range(1, 8):
            pro_content += f"{percentage_bar(self.pro_tier_maps[i] / self.maps.tier[i], bar_length=15, fill_char=emojis[i], empty_char='⬛', show_percentage=False, show_brackets=False)}"
