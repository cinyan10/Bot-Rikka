...
from functions.steam.steam import convert_steamid, is_in_group, is_user_in_group
...
async def get_whitelisted(ctx):
    ...
    if ban_status['vac_banned']:
        await ctx.send(embed=Embed(title="You have been banned by VAC!!!", colour=discord.Colour.red()))
        return
    elif ban_status['game_ban_count'] > 1:
        await ctx.send(embed=Embed(title=f"You have been banned by {ban_status['game_ban_count']} games!!!", colour=discord.Colour.red()))
        return
    else:
    ...
    is_group = is_user_in_group(steamid64)
